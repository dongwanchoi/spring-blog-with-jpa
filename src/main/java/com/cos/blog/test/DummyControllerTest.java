...
public class DummyControllerTest {
	...
	// save함수는 id를 전달하고 해당 id에 대한 데이터가 없으면 insert를 해요.
	// email, password
	@Transactional //함수 종료시에 자동 commit이 됨. (더티체킹 : 값 변경을 감지함)
	@PutMapping("/dummy/user/{id}") // json데이터로  가져옴
	...
	@GetMapping("/dummy/users") //select * from
	public List<User> list(){
		return userRepository.findAll();
	}
	
	//한 페이지당 2건에 데이터를 리턴받아 볼 예정
	@GetMapping("/dummy/user") //페이징 처리
	...
	public User detail(@PathVariable int id) { //상세보기
		...
		// 그럼 return null이 리턴이 되잖아, 그럼 프로그램에 문제가 있지 않겠어?
		// Optional로 너의 User 객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return해. 
		
		// userRepository.findById(id).get(); //null 절대 없어. 그냥 가져와.
//		User user = userRepository.findById(id).orElseGet(new Supplier<User>() {//그 값이 없으면 니가 객체 하나 만들어서 user에 넣어줘 그럼 user가 null이 아니잖아.
//			@Override
//			public User get() {
//				return new User();
//			}
//		});
	...
	@PostMapping("/dummy/join") //폼태그로 값 가져옴
